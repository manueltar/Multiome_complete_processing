...
do
    ...
    Rscript_Seurat_first_pass=$(echo "$Rscripts_path""403_Seurat_first_pass.R")
    ...
    myjobid_Seurat_first_pass=$(sbatch --job-name $name_Seurat_first_pass --output=$outfile_Seurat_first_pass --partition=cpuq --time=24:00:00 --nodes=1 --ntasks-per-node=24 --mem-per-cpu=4096 --parsable --wrap="Rscript $Rscript_Seurat_first_pass --sample_name $sample_name --master_path $master_path --rna_min_features $rna_min_features --atac_min_fragments $atac_min_fragments --MITO_max $MITO_max --Threshold_UMIS_per_cell $Threshold_UMIS_per_cell --Uniquely_genotyped_larry_barcodes_assignments $Uniquely_genotyped_larry_barcodes_assignments --type $type --out $output_dir")
