...
data_wrangling = function(option_list)
{
  ...
  DE_genes<-as.data.frame(read.table(file=opt$DE_genes, sep="\t", header=T), stringsAsFactors=F)
  ...
  SIG<-DE_genes[which(DE_genes$padj <= 0.05),]
  ...
  adata <- RegionStats(adata, genome = BSgenome.Hsapiens.UCSC.hg38, assay='ATAC_by_refined_annotation')
  
  
  adata <- LinkPeaks(
    object = adata,
    peak.assay = "ATAC_by_refined_annotation",
