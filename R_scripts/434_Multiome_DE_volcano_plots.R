...
volcano_plots_function = function(option_list)
{
  ...
  #### READ and transform DE_result ----
  
  
  DE_result<-readRDS(file=opt$DE_result)
  ...
  array_identities<-levels(DE_result$identity)
     ...
     if(dim(SIG_genes)[1] >0){
       ...
       if (file.exists(path_contrast_sel)){
         ...
         if(dim(SIG_genes)[1] <= 50){
           ...
           volcano_plot<-ggplot(data=DE_result_sel_contrast_sel,
             ...
             geom_point(data=DE_result_sel_contrast_sel[which(DE_result_sel_contrast_sel$minuslog10padj < 1.3 & DE_result_sel_contrast_sel$abslogfc < 0.1),],
                        color="gray",fill="gray", stroke=0.2, shape=21, size=2)+
             geom_point(data=DE_result_sel_contrast_sel[which(DE_result_sel_contrast_sel$minuslog10padj >= 1.3 & DE_result_sel_contrast_sel$log2FoldChange <= -0.1),],
           ...
           volcano_plot <-volcano_plot+
             geom_text_repel(data=DE_result_sel_contrast_sel[which(DE_result_sel_contrast_sel$minuslog10padj >= 1.3 & DE_result_sel_contrast_sel$log2FoldChange <= -0.1),],
                             aes(x=log2FoldChange,
                                 y=minuslog10padj,
                                 label=gene),
                             family="sans",
       ...
       svgname<-paste("volcano_plot",".svg",sep='')
...
main = function() {
  ...
  cat(paste(gsub("--file=", "", cmd_line[4], fixed=T),
            ...
            "\n\n"))
  option_list <- list(
    make_option(c("--DE_result"), type="character", default=NULL, 
                metavar="type", 
