...
DA_function = function(option_list)
{
  ...
  # Join layers of ATAC_by_rpca_refined_annotation after the integration ----------------------
  ...
  matrix_ATAC<-LayerData(object = adata, assay = "ATAC_by_rpca_refined_annotation", layer = "counts")
  ...
  aggr_counts <- Seurat2PB(ATAC_object, sample="sample_id", cluster="rpca_annot_majority_vote")
  ...
  for(i in 1:length(array_identities)){
    ...
    nor_counts.m<-melt(nor_counts, id.vars='Peak_ID', variable.name="sample_id_seurat_identity_id", value.name="count")
  ...
  write.table(DA_results, file="DA_results.tsv", sep="\t", quote=F, row.names = F)
