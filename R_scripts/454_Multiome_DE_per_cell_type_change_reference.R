...
DE_function = function(option_list)
{
  ...
  adata$sample_id<-droplevels(interaction(adata$clone_line, sep="_"))
  ...
  matrix_RNA<-LayerData(object = adata, assay = "RNA", layer = "counts")
  ...
  cat(sprintf(as.character(names(summary(metadata$rpca_annot_majority_vote)))))
  cat("\n")
  cat(sprintf(as.character(summary(metadata$rpca_annot_majority_vote))))
  cat("\n")
  ...
  cluster_names <- levels(metadata[,which(colnames(metadata) == 'rpca_annot_majority_vote')])
  ...
  groups <- metadata[,c(which(colnames(metadata) == 'sample_id'),which(colnames(metadata) == 'rpca_annot_majority_vote'))]
  
  cat("groups_0\n")
  cat(str(groups))
  cat("\n")
  
  aggr_counts <- Seurat2PB(RNA_object, sample="sample_id", cluster="rpca_annot_majority_vote")
  ...
  t <- table(metadata$sample_id,
             metadata$rpca_annot_majority_vote)
  ...
  for (i in 1:length(counts_ls)) {
    ...
    df <- data.frame(rpca_annot_majority_vote_sample_id = colnames(counts_ls[[i]]))
    ...
    df$rpca_annot_majority_vote_id <- tstrsplit(df$rpca_annot_majority_vote_sample_id, "_cluster")[[2]]
    df$sample_id  <- tstrsplit(df$rpca_annot_majority_vote_sample_id, "_cluster")[[1]]
    
    identity_sel<-unique(df$rpca_annot_majority_vote_id)
    ...
    idx <- which(colnames(t) == unique(df$rpca_annot_majority_vote_id))
  ...
  array_identities<-levels(adata@meta.data$rpca_annot_majority_vote)
  ...
  for(i in 1:length(array_identities)){
    ...
    G_in_CT<-levels(droplevels(metadata_df$Genotype))
    ...
    if(DEBUG == 1){
      ...
      cat(str(G_in_CT))
      cat("\n")
      cat(sprintf(as.character((G_in_CT))))
      cat("\n")
    }
    
    
    FLAG_presence_of_level<-length(G_in_CT[which(G_in_CT == 'DNMT3A')])
    
    if(DEBUG == 1){
      
      cat("FLAG_presence_of_level_0\n")
      cat(str(FLAG_presence_of_level))
      cat("\n")
    }
    
    if(FLAG_presence_of_level >0){
      
      metadata_df$Genotype<-as.character(metadata_df$Genotype)
      metadata_df$Genotype<-factor(metadata_df$Genotype)
      metadata_df$Genotype<-relevel(metadata_df$Genotype, ref='DNMT3A') ### Nothing works with ordered factors
      ...
      row.names(metadata_df)<-metadata_df$rpca_annot_majority_vote_sample_id
      ...
      nor_counts.m<-melt(nor_counts, id.vars='gene', variable.name="sample_id_seurat_identity_id", value.name="count")
  ...
  DE_results$contrast<-factor(DE_results$contrast,
                              levels=c('Genotype_rs62237617_vs_DNMT3A','Genotype_wt_vs_DNMT3A','Genotype_rs62237617_DNMT3A_vs_DNMT3A'),
                              ordered=TRUE)
  ...
  write.table(norcounts_FINAL, file="norcounts_FINAL.tsv", sep="\t", quote=F, row.names = F)
  saveRDS(norcounts_FINAL, file="norcounts_FINAL.rds")
